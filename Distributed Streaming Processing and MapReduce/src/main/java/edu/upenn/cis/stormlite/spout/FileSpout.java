...
public abstract class FileSpout implements IRichSpout {
    ...
    public void open(Map conf, TopologyContext context,
        ...
        try {
        	// get dir
	        String storageDir = WorkerServer.getConfig().get("storageDir");
			...
			String dir = Paths.get(storageDir, inputDir).toAbsolutePath().toString();
			File[] files = new File(dir).listFiles();
			
			for (File f : files) {
				if(f.getName().endsWith("." + conf.get("workerIndex"))) {
					filename = f.getName();
